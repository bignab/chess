  ...
  describe 'pawn_moves' do
    context 'when white' do
      ...
      it 'on square [6,0]' do
        ...
        expect(subject.pawn_moves([6, 0], 'white', false, subject.board, subject.board.squares[0][6])).to match_array(correct_legal_moves) # rubocop:disable Layout/LineLength
      end
    end
    context 'when black' do
      it 'on square [4,5]' do
        correct_legal_moves = [[4, 5]]
        expect(subject.pawn_moves([3, 5], 'black', true, subject.board, subject.board.squares[0][1])).to match_array(correct_legal_moves) # rubocop:disable Layout/LineLength
      end
      it 'on square [1,0]' do
        correct_legal_moves = [[2, 0], [3, 0]]
        expect(subject.pawn_moves([1, 0], 'black', false, subject.board, subject.board.squares[0][1])).to match_array(correct_legal_moves) # rubocop:disable Layout/LineLength
      end
    end
  end
end
