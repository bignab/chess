...
class Parser
  ...
  def pawn_move(input, promo = nil)
    return pawn_move(input[0..-3], input[-1]) if input.length > 3 && input[-2] == '=' && input[-1] =~ /[QRBN]/
  ...
  # Returns an array with five elements: 0[type], 1[move], 2[disamb], 3[captures?], 4[promotion].
